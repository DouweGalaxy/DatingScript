...
{	$id = $arr;
	$displayorder = $osDB->getOne( 'SELECT displayorder FROM ! WHERE id = ?', array( CALENDARS_TABLE, $id ) );
	...
	while ( $i <= $maxorder['maxorder'] ){
		$osDB->query( 'UPDATE ! SET displayorder = ? WHERE displayorder = ?', array( CALENDARS_TABLE, $i - 1, $i ) );
		$i++;
	}
	// Delete events
